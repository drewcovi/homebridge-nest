...
NestThermostatAccessory.prototype.setTargetHeatingCooling = function (targetHeatingCooling, callback) {
  ...
  // simulate a call to revert mode if in eco mode and modechange was requested
  const currentMode = this.getCurrentHeatingCooling();
  if(this.device.hvac_mode === 'eco' && targetHeatingCooling !== currentMode) {
    setTimeout(function(){
      this.getService(Service.Thermostat)
         .setCharacteristic(Characteristic.TargetHeatingCoolingState, currentMode);
    },1000);
